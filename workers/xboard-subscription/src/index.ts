...
function plain(user: any, servers: any[]) {
  return servers.map(s => {
    ...
    if (s.type === "shadowsocks") return `ss://${b64(`${ps.cipher || "aes-128-gcm"}:${user.uuid}@${s.host}:${s.port}`)}#${encodeURIComponent(s.name)}`;
    return `vmess://${b64(JSON.stringify({ v: "2", ps: s.name, add: s.host, port: String(s.port), id: user.uuid, aid: "0", net: ps.network || "tcp", type: "none", host: ps.host || "", path: ps.path || "", tls: ps.tls ? "tls" : "" }))}`;
  }).join("\n");
}
...
async function build(request: Request, env: Env, token: string) {
  ...
  const servers = (await env.XBOARD_DB.prepare("SELECT * FROM v2_server WHERE enabled = 1 AND show = 1 ORDER BY sort DESC, id ASC").all<any>()).results || [];
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const userVersion = await env.XBOARD_KV.get(`user_version:${token}`) || "0";
    const client = clientOf(request);

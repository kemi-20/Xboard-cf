...
export async function settleOrder(
  ...
  const orderGuard = "EXISTS (SELECT 1 FROM v2_order WHERE id=? AND status IN (0,1))";
  ...
  if (period === "reset_traffic") {
    ...
    statements.push(env.XBOARD_DB.prepare(`UPDATE v2_user SET
      ...
      u=?,d=?,balance=COALESCE(balance,0)+?,last_reset_at=?,next_reset_at=?,
      reset_count=COALESCE(reset_count,0)+?,updated_at=?
      WHERE id=? AND ${orderGuard}`)
      .bind(
        plan.id, plan.group_id, Number(plan.transfer_enable || 0) * 1073741824,
        plan.speed_limit ?? null, plan.device_limit ?? null, expiredAt,
        resetTraffic ? 0 : Number(user.u || 0), resetTraffic ? 0 : Number(user.d || 0),
        Number(order.surplus_credit || 0), resetTraffic ? timestamp : user.last_reset_at ?? null,
        nextReset, resetTraffic ? 1 : 0, timestamp, user.id, order.id
      ));
  }
  if (resetTraffic) {
    ...
    statements.push(env.XBOARD_DB.prepare(`INSERT INTO v2_traffic_reset_logs(
      ...
      .bind(
        user.id, resetTypes[method] || "manual", Number(user.u || 0), Number(user.d || 0),
        Number(user.u || 0), Number(user.d || 0), Number(user.u || 0) + Number(user.d || 0),
        JSON.stringify({ order_id: order.id, trade_no: order.trade_no, event_id: orderEventId || null }),
        timestamp, timestamp, order.id
      ));
  }
  statements.push(env.XBOARD_DB.prepare(`UPDATE v2_order SET status=3,paid_at=?,callback_no=?,updated_at=?
    WHERE id=? AND status IN (0,1)`).bind(timestamp, callbackNo, timestamp, order.id));

...
const inflight = new Map<string, Promise<JsonRow[]>>();
...
function requestShape(input: JsonRow) {
  ...
  const start = integer(input.start, current - 86400, 0, current + 86400);
  const end = integer(input.end, current, start + 1, current + 86400);
...
async function queryAnalytics(env: AnalyticsEnv, query: string) {
  if (!env.ANALYTICS_API_TOKEN || !env.CLOUDFLARE_ACCOUNT_ID) throw new Error("ANALYTICS_NOT_CONFIGURED");
...
async function cachedQuery(env: AnalyticsEnv, kind: QueryKind, input: JsonRow, ttl: number, ctx?: ExecutionContext) {
  const key = cacheKey(kind, input);
  const current = Date.now();
  const local = memory.get(key);
  if (local && local.expiresAt > current) return local.value;
  ...
  if (cache) {
    try {
      ...
      if (hit) {
        ...
        memory.set(key, { value, expiresAt: current + ttl * 1000 });
        return value;
      }
    } catch {}
  }
  ...
  if (!pending) {
    pending = queryAnalytics(env, queryFor(kind, input));
    inflight.set(key, pending);
  }
  try {
    const value = normalize(kind, await pending);
    memory.set(key, { value, expiresAt: current + ttl * 1000 });
    if (cache) {
      const write = cache.put(request, new Response(JSON.stringify(value), {
...
export async function analyticsData<T>(env: AnalyticsEnv, path: string, input: JsonRow, ctx?: ExecutionContext): Promise<T | null> {
  const route = ROUTES[path];
  if (!route) return null;
  try {
    return await cachedQuery(env, route.kind, input, route.ttl, ctx) as T;
  } catch {
    return null;
  }
}

export const __test = { requestShape, queryFor, normalize, queryAnalytics, cacheKey };

...
首次登录后请立即修改默认密码。首次部署 Action 和 `schema/seed.sql` 只会在该邮箱不存在时创建默认管理员；重复执行不会覆盖已有密码、权限、套餐或其他账号字段。
...
支付配置保存在 `v2_payment.config`，管理员读取配置和 SQLite 完整导出均会包含私钥、API Key 与 Webhook Secret。管理员审计日志会递归脱敏这些字段，但导出的数据库不会脱敏，必须作为敏感凭据安全保管。已有渠道不能原地更换 Provider；轮换密钥或通知域名前必须先停用渠道，保存新配置时会解除待支付订单绑定并清理未支付会话，已支付历史记录保持不变。自定义网关和通知域名只接受公开 HTTPS 地址；Provider 请求有超时、响应大小和返回结构限制。
